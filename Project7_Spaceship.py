...
class Ship:
    ...
    def shoot(self):
        ...
        a_missile.vel[0] = self.vel[0] + (angle_to_vector(self.angle)[0] * 2)
        a_missile.vel[1] = self.vel[1] + (angle_to_vector(self.angle)[1] * 2)
...
class Sprite:
    def __init__(self, pos, vel, ang, ang_vel, image, info, sound = None):
        ...
        self.vel = [vel[0],vel[1]]
        self.angle = ang
        self.angle_vel = ang_vel * random.choice([-1, 1]) * (random.random() + 1)
        self.image = image
        self.image_center = info.get_center()
        self.image_size = info.get_size()
        self.radius = info.get_radius()
...
def draw(canvas):
    ...
    canvas.draw_text("Score", [WIDTH - 125, 50], 25, "White")
    canvas.draw_text(str(score), [WIDTH - 125, 75], 25, "White")
...
def rock_spawner():
    ...
    angle_vel = 5/60.0 * random.choice([-1, 1]) * random.randrange(1, 5)
...
def keydown(key):
    ...
    if key == simplegui.KEY_MAP['up']:
        my_ship.thrust_on(my_ship.thrust)
    if key == simplegui.KEY_MAP['left']:
        my_ship.angle_vel = -4 / 60.0
...
my_ship = Ship([WIDTH / 2, HEIGHT / 2], [0.0, 0.0], 0, ship_image, ship_info)
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [1, 1], 0, 5/60.0, asteroid_image, asteroid_info)
